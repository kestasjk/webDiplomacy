	...
	static function forumNewExtern()
	{
		...
		while($t = $DB->tabl_hash($tabl))
		{
			...
			$buf .= '<span style=\'font-size:90%\'>'.($t['topic_posts_approved']>1?'Re: ':'New: ').'</span>'
					...
					$buf .= l_t('<span class="forum-preview-span">%s replies','<strong>'.($t['topic_posts_approved']-1).'</strong>');
					$buf .= ', '.l_t('%s views</span>','<strong style=\'content: "\f14c"\'>'.($t['topic_views']-1).'</strong>');
					$buf .= '</div>';
