...
function Order(orderData) {
  ...
  this.getDataArray = function () {
    var da = {
      ...
      unitID: this.unitID,
      type: this.type,
      toTerrID: this.toTerrID,
      fromTerrID: this.fromTerrID,
  ...
  this.load = function () {
    ...
    if (this.toTerrID != null)
      this.ToTerritory = Territories.get(this.toTerrID);

    if (this.fromTerrID != null)
      this.FromTerritory = Territories.get(this.fromTerrID);

    $("orderID" + this.id).replace(
      new Element("div", { id: "orderID" + this.id })
    ); //'<div id="orderID'+this.id+'"></div>');
    ...
    this.orderSegmentNames.map(function (n) {
      var el = new Element("span", { class: "orderSegment" });
    ...
    this.requirements.map(function (n) {
      this.reHTML(n);
    }, this);
    this.reHTML("orderEnd");

    if (!this.isChanged) this.setSelectsGreen();
  ...
  this.setUnitIconArea = function (newIcon) {
    ...
    if (this.currentUnitIcon != false)
      $("orderID" + this.id).removeClassName(
        this.currentUnitIcon.toLowerCase()
      );

    $("orderID" + this.id).addClassName(newIcon.toLowerCase());
    this.currentUnitIcon = newIcon;

    this.unitIconArea.update(
      '<img src="' +
        l_s("images/" + newIcon + ".png") +
        '" alt="' +
        l_t(newIcon) +
        '" />'
    );
  ...
  this.checkComplete = function () {
    this.setComplete(
      this.requirements.all(function (r) {
        return this[r] != "";
      }, this)
    );
  ...
  this.wipe = function (toWipe) {
    toWipe.map(function (w) {
      switch (w) {
        case "type":
          this.type = "";
          this.typeChoices = undefined;
          return;
        case "toTerrID":
          this.toTerrID = "";
          this.ToTerritory = undefined;
          this.toTerrChoices = undefined;
          return;
        case "fromTerrID":
          this.fromTerrID = "";
          this.FromTerritory = undefined;
  ...
  this.fromrequirements = function (arr) {
    return this.requirements.select(function (r) {
      return arr.member(r);
    });
  };
  ...
  this.isValid = function (name, value) {
    ...
    switch (name) {
      case "type":
        choices = $H(this.typeChoices).keys();
        break;
      case "toTerrID":
        choices = $H(this.toTerrChoices).keys();
        break;
      case "fromTerrID":
        choices = $H(this.fromTerrChoices).keys();
        break;
      case "viaConvoy":
        choices = $H(this.viaConvoyChoices).keys();
        break;
    }

    if (
      Object.isUndefined(choices) ||
      choices.length == 0 ||
      !choices.member(value)
    )
      return false;
    else return true;
  };
  ...
  this.updateValue = function (name, newValue) {
    ...
    switch (name) {
      case "type":
        ...
        updatedChoices = this.updateChoices(
          this.fromrequirements(["toTerrID", "viaConvoy"])
        );
        break;
      case "toTerrID":
        ...
        updatedChoices = this.updateChoices(
          this.fromrequirements(["fromTerrID", "viaConvoy"])
        );
        break;
      case "fromTerrID":
    ...
    updatedChoices.map(function (c) {
      this.reHTML(c);
    }, this);
  };

  this.orderSegmentNames = [
    "orderBegin",
    "type",
    "toTerrID",
    "fromTerrID",
  ...
  this.orderSegmentHTML = function (name) {
    switch (name) {
      ...
      case "toTerrID":
        return this.toTerrHTML();
      case "fromTerrID":
        return this.fromTerrHTML();
      case "viaConvoy":
        return this.viaConvoyHTML();
      case "orderEnd":
        return this.endHTML();
    }
  };
  ...
  this.alterOrderSegment = function (name, HTML) {
    ...
    OrderSegment.select("select").map(function (e) {
      e.observe("change", this.onChange.bindAsEventListener(this));
    }, this);
  };

  this.updateChoices = function (choices) {
    return choices.select(function (c) {
      return this.updateChoice(c);
    }, this);
  };
  this.updateChoice = function (name) {
    ...
    switch (name) {
      case "type":
        ...
        newChoices = this.updateTypeChoices();
        break;
      case "toTerrID":
        currentValue = this.toTerrID;
        newChoices = this.updateToTerrChoices();
        break;
      case "fromTerrID":
        currentValue = this.fromTerrID;
        newChoices = this.updateFromTerrChoices();
        break;
      case "viaConvoy":
        currentValue = this.viaConvoy;
        newChoices = this.updateViaConvoyChoices();
        break;
      default:
        return false;
    }
    ...
    if (newChoices.values().length == 1) {
      ...
      else if (onlyValue != currentValue) this.updateValue(name, onlyValue);
    } else if (newChoices.values().length == 0) this.updateValue(name, "");
  ...
  this.formDropDown = function (name, aoptions, value) {
    ...
    if (Object.isUndefined(aoptions)) {
      return "";
    }
    ...
    if (OrdersHTML.finalized) return " " + options.get(value) + " ";
    else {
      ...
      optArray.forEach(function (pair) {
        if (
          !(Object.isUndefined(pair[0]) || pair[0] == "undefined") &&
          pair[0] != value
        )
          html =
  ...
  this.setResult = function (Result) {
    ...
    if (Result.status == "Complete") {
      if (!this.checkComplete()) {
        message = l_t(
          "Incompatibility between order and requirements, please report via the forums!"
        );
        icon = "alert";
      } else {
        this.setSelectsGreen();
        if (OrdersHTML.finalized) icon = "tick";
        else icon = "tick_faded";
      }
      ...
      message =
        " " +
        (Result.notice == null ? l_t("Undefined error") : l_t(Result.notice)) +
        "<br />";
    }

    this.setMessageSpan(
      '<img src="' +
        l_s("images/icons/" + icon + ".png") +
        '" alt="' +
        l_t(icon) +
        '" /> ' +
        message
  ...
  this.arrayToChoices = function (arr) {
    ...
    arr.map(function (c) {
      choices.set(c, l_t(Territories.get(c).name));
    });

    return choices;
  };

  this.fNothing = function () {
    return;
  };
}
