  ...
  MyOrders.map(function (OrderObj) {
    ...
    OrderObj.postUpdate = function () {
      ...
      if (this.isComplete) {
        if (
          this.type == "Move" &&
          this.Unit.convoyOptions.any(function (c) {
            return c == this.toTerrID;
          }, this)
        ) {
          convoyPath = this.Unit.ConvoyGroup.pathArmyToCoast(
            this.Unit.Territory,
            this.ToTerritory
          );
        } else if (
          this.type == "Support move" &&
          this.Unit.convoyOptions.any(function (c) {
            return c == this.fromTerrID;
          }, this)
        ) {
          if (this.Unit.type == "Fleet") {
            convoyPath =
              this.ToTerritory.ConvoyGroup.pathArmyToCoastWithoutFleet(
                this.FromTerritory,
                this.ToTerritory,
                this.Unit.Territory
              );
          } else if (this.ToTerritory.convoyLink) {
            convoyPath = this.ToTerritory.ConvoyGroup.pathArmyToCoast(
              this.FromTerritory,
              this.ToTerritory
            );
          }
          ...
          convoyPath = this.Unit.ConvoyGroup.pathArmyToCoastWithFleet(
            this.FromTerritory,
            this.ToTerritory,
            this.Unit.Territory
          );
      ...
      if (false && this.autoFill) {
        var thisOrder = this;
        var filterSet = function (fFilter, fSet) {
          MyOrders.select(fFilter).map(fSet);
        };

        if (Object.isUndefined(this.setAndShow)) {
          MyOrders.map(function (o) {
            o.setAndShow = function (n, v) {
              o.inputValue(n, v);
              o.reHTML(n);
            };
          });
          ...
          case "Support hold":
            if (this.isComplete) {
              filterSet(
                function (o) {
                  return (
                    o.Unit.Territory.id == thisOrder.ToTerritory.id &&
                    o.type == "Move"
                  );
                },
                function (o) {
            ...
            if (!Object.isUndefined(this.FromTerritory)) {
              ...
              MyOrders.map(function (o) {
                ...
                convoyingArmyList
                  .select(function (o) {
                    return (
                      Object.isUndefined(o.ToTerritory) ||
                      o.ToTerritory.id != thisOrder.ToTerritory.id
                    );
                  })
                  .map(function (o) {
                    o.setAndShow(
                      "toTerrID",
                      thisOrder.ToTerritory.id.toString()
                    );
                  });
              }, this);
            }
    ...
    OrderObj.updaterequirements = function () {
      ...
      this.wipe(
        oldrequirements.reject(function (r) {
          return this.requirements.member(r);
        }, this)
      );
    };
    ...
    OrderObj.updateTypeChoices = function () {
      this.typeChoices = {
        Hold: l_t("hold"),
        Move: l_t("move"),
        "Support hold": l_t("support hold"),
        "Support move": l_t("support move"),
      };
      ...
      switch (this.type) {
        case "Move":
          ...
          if (this.Unit.type == "Army" && this.Unit.Territory.type == "Coast") {
            var ttac = new Hash();
            var armylocalchoices =
              this.Unit.getMovableTerritories().pluck("id");
            this.toTerrChoices.map(function (c) {
              if (armylocalchoices.member(c))
                ttac.set(c, l_t(Territories.get(c).name));
              else
                ttac.set(
                  c,
                  l_t(Territories.get(c).name) + " " + l_t("(via convoy)")
                );
            });
          ...
          break;
        case "Support hold":
          this.toTerrChoices = this.Unit.getSupportHoldChoices();
          break;
        case "Support move":
          this.toTerrChoices = this.Unit.getSupportMoveToChoices();
          break;
        case "Convoy":
          this.toTerrChoices = this.Unit.getConvoyToChoices();
          break;
        default:
    ...
    OrderObj.updateFromTerrChoices = function () {
      if (Object.isUndefined(this.ToTerritory)) {
        ...
        switch (this.type) {
          case "Support move":
            this.fromTerrChoices = this.Unit.getSupportMoveFromChoices(
              this.ToTerritory
            );
            break;
          case "Convoy":
            this.fromTerrChoices = this.Unit.getConvoyFromChoices(
              this.ToTerritory
            );
            break;
          default:
    ...
    OrderObj.updateViaConvoyChoices = function () {
      if (this.type != "Move" || this.toTerrID == "")
        ...
        !this.Unit.ConvoyGroup.Coasts.member(this.ToTerritory)
      )
        this.viaConvoyChoices = new Hash({ No: l_t("via land") });
      else if (this.Unit.getMovableTerritories().member(this.ToTerritory))
        this.viaConvoyChoices = new Hash({
          Yes: l_t("via convoy"),
          No: l_t("via land"),
        });
      else this.viaConvoyChoices = new Hash({ Yes: l_t("via convoy") });
    ...
    OrderObj.beginHTML = function () {
      return (
        l_t(
          "The %s at %s",
          l_t(this.Unit.type.toLowerCase()),
          l_t(this.Unit.Territory.name)
        ) + " "
      );
    };
    ...
    OrderObj.toTerrHTML = function () {
      var toTerrID = this.formDropDown(
        "toTerrID",
        this.toTerrChoices,
        this.toTerrID
      );
      ...
      var ToUnitType;
      if (
        Object.isUndefined(this.ToTerritory) ||
        Object.isUndefined(this.ToTerritory.Unit)
      )
        ToUnitType = "unit";
      else ToUnitType = this.ToTerritory.Unit.type.toLowerCase();

      switch (this.type) {
        // toTerrID is retrieved from the list of choices, so is already translated
        case "Move":
          return l_t(" to %s ", toTerrID);
        case "Support hold":
          return l_t(" the %s in %s ", l_t(ToUnitType), toTerrID);
        case "Support move":
          return l_t(" to %s ", toTerrID);
        case "Convoy":
          return l_t(" an army to %s ", toTerrID);
        default:
          return "";
      }
    };
    OrderObj.fromTerrHTML = function () {
      ...
      var fromTerrID = this.formDropDown(
        "fromTerrID",
        this.fromTerrChoices,
        this.fromTerrID
      );

      switch (this.type) {
        case "Support move":
          return l_t(" from %s ", fromTerrID);
        case "Convoy":
          return l_t(" from %s ", fromTerrID);
        default:
          return "";
    ...
    OrderObj.viaConvoyHTML = function () {
      if (Object.isUndefined(this.viaConvoyChoices)) return "";
      else if (this.viaConvoyChoices.values().length == 1)
        return (
          '<input type="hidden" name="orderForm[' +
          this.id +
          '][viaConvoy]" value="' +
          this.viaConvoyChoices.values()[0] +
          '" />'
        );
      else
        return this.formDropDown(
          "viaConvoy",
          this.viaConvoyChoices,
          this.viaConvoy
        );
    };
