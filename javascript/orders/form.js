...
function OrdersHTMLFormClass() {
  ...
  this.formInit = function (context, contextKey) {
    ...
    if (this.finalized) this.setFinalized();
    else this.setUnfinalized();

    this.OrdersIndex = new Hash();
    MyOrders.map(function (o) {
      this.OrdersIndex.set(o.id, o);
    }, this);

    if (MyOrders.length == 0) {
      this.orderDiv.update(
        '<p class="notice">' +
          l_t("You have no orders to fill for this phase.") +
          "</p>"
      );
    } else {
      this.updateFormButtons();
      if (this.UpdateButton != null)
        this.UpdateButton.observe(
          "click",
          this.onSave.bindAsEventListener(this)
        );
      this.FinalizeButton.observe(
        "click",
        this.onLock.bindAsEventListener(this)
      );
    }
  };
  ...
  this.sendUpdates = function (url) {
    var up = MyOrders.select(function (o) {
      return o.isComplete;
    }).map(function (o) {
      return o.getDataArray();
    }, this);
    ...
    new Ajax.Request(url, {
      ...
      parameters: {
        orderUpdates: up.toJSON(),
        context: Object.toJSON(this.context),
  ...
  this.onLock = function (event) {
    this.sendUpdates(
      "ajax.php?" + (this.finalized ? "notready" : "ready") + "=on"
    );
  ...
  this.setUnfinalized = function () {
    ...
    MyOrders.map(function (o) {
      o.setSelects(function (e) {
        if (!e.hasClassName("orderDisabled")) e.enable();
      });
    });
  ...
  this.onSuccess = function (response) {
    if (response.headerJSON == null) {
      ...
      if (!Object.isUndefined(response.headerJSON.newContext)) {
        ...
        if (tmpFinalized && !this.finalized) this.setFinalized();
        else if (!tmpFinalized && this.finalized) this.setUnfinalized();
      }

      if (response.headerJSON.statusIcon.length > 0)
        $$(".member" + this.context.memberID + "StatusIcon").map(function (e) {
          e.update(response.headerJSON.statusIcon);
        });

      if (response.headerJSON.statusText.length > 0)
        $$(".member" + this.context.memberID + "StatusText").map(function (e) {
          e.update(response.headerJSON.statusText);
        });

      if (response.headerJSON.notice.length > 0)
        this.ordersNoticeArea.update(
          response.headerJSON.statusIcon +
            " " +
            response.headerJSON.notice +
            "<br />"
        );
      else this.ordersNoticeArea.update(response.headerJSON.statusIcon);
  ...
  this.updateFormButtons = function () {
    if (
      MyOrders.pluck("isChanged").any(function (c) {
        return c;
      })
    ) {
    ...
    if (
      this.finalized ||
      MyOrders.pluck("isComplete").all(function (c) {
        return c;
      })
    )
      this.buttonOn("FinalizeButton");
    else this.buttonOff("FinalizeButton");
  };
}
...
if (window.addEventListener) {
  ...
  console.log(
    "Added onbeforeunload_unsavedorders listener via addEventListener"
  );
} else {
