...
if( $User->type['Guest'] )
	print libHTML::pageTitle(l_t('Blog'), l_t('webDiplomacy blog where to get the latest news and updates.'));
else
...
print '
	...
	else
	{
		...
		<div class="message-body postbox" style="padding-top:0; padding-left:auto; padding-right:auto">
			...
			<form class="safeForm" action="blog.php#postbox" method="post"><p>
	...
	<div class="message-foot threadalternate1">
		<form action="blog.php" method="get" onsubmit="$(\'forumPostbox\').hide(); $(\'forumOpenPostbox\').show(); return false;">
	...
	if( $User->type['Admin'] || $User->type['ForumModerator'] || $User->type['Moderator'] ) 
	{
		print '<div id="forumOpenPostbox" style="'.($postboxopen?libHTML::$hideStyle:'').'" >
			<form action="blog.php#postbox" method="get" onsubmit="$(\'forumPostbox\').show(); $(\'forumOpenPostbox\').hide(); return false;">
			<p style="padding:5px;">
...
while( $message = $DB->tabl_hash($tabl) )
{
	...
	if( $User->type['User'] ) {
		...
		$toggleMuteURL = 'blog.php?toggleMuteThreadID='.$message['id'].'&rand='.rand(1,99999).'#'.$message['id'];
	...
	if( $message['id'] == $viewthread )
	{
		...
		{
			print '<form action="blog.php#'.$replyID.'" method="get">
	...
	if ( $message['id'] == $viewthread )
	{
		if($User->type['User'] && !isset($postLockedReason) )
		{
			print '<div class="postbox">'.
				...
				'<form class="safeForm" action="./blog.php?newsendtothread='.$viewthread.'&amp;viewthread='.$viewthread.'#postbox" method="post">
	...
	if ( $message['id'] == $viewthread )
	{
		print '<form action="blog.php#'.$message['id'].'" method="get">
						<input type="hidden" name="viewthread" value="0" />
	...
	{
		print '<a href="blog.php?viewthread='.$message['id'].'#'.$message['id'].'" '.
