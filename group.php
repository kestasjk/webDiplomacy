...
if( $GroupProfile->gameID && $User->type['Moderator'] )
{
	// If we're a moderator print extra info about the game and user
	require_once(l_r('objects/game.php'));
	require_once(l_r('gamepanel/game.php'));
	$Variant=libVariant::loadFromGameID($GroupProfile->gameID);
	$G = $Variant->panelGame($GroupProfile->gameID);
	print $G->summary(false);
	
	print ' <strong>Created during turn:</strong> '.$GroupProfile->gameTurn.' - '.$Variant->turnAsDate($GroupProfile->gameTurn);
...
{	
	...
	print '<div class = "profile_content_show">';
			...
			$multiAccountParams = substr($multiAccountParams, 0, strlen($multiAccountParams)-3);
