...
class panelGameBoard extends panelGame
{
	function mapHTML() 
	{
		...
		$map = '
		<div id="mapstore">
			<img id="mapImage" src="'.$smallmapLink.'" alt=" " title="'.l_t('The small map for the current phase. If you are starting a new turn this will show the last turn\'s orders').'" style="width: auto; max-width: 100%;" />
			<p class="lightgrey" style="text-align:center">
				<a class="mapnav" href="#" onClick="loadMap('.$this->id.','.$mapTurn.',-1); return false;">
                      <img id="Start" src="'.l_s('images/historyicons/Start_disabled.svg').'" alt="'.l_t('Start').'" title="'.l_t('View the map from the first turn').'" /></a>
				<a class="mapnav" href="#" onClick="loadMapStep('.$this->id.','.$mapTurn.',-1); return false;"><img id="Backward" src="'.l_s('images/historyicons/Backward_disabled.svg').'" alt="'.l_t('Backward').'" title="'.l_t('View the map from the previous turn').'" /></a>
                ...
                <a class="mapnav" href="#" onClick="toggleMoves('.$this->id.','.$mapTurn.'); return false;"><img id="NoMoves" src="images/historyicons/hidemoves.svg" alt="NoMoves" title="Toggle movement lines" />
                </a>                 -->
			   <a id="LargeMapLink" class="mapnav" href="'.$largemapLink.'" target="_blank" class="light"><img src="'.l_s('images/historyicons/external.svg').'" alt="'.l_t('Open large map').'" title="'.l_t('This button will open the large map in a new window. The large map shows all the moves, and is useful when the small map isn\'t clear enough.').'" /></a></span>
                     
				<a class="mapnav" href="#" onClick="loadMapStep('.$this->id.','.$mapTurn.',1); return false;"><img id="Forward" src="'.l_s('images/historyicons/Forward_disabled.svg').'" alt="'.l_t('Forward').'" title="'.l_t('View the map from the next turn').'" /></a>
				<a class="mapnav" href="#" onClick="loadMap('.$this->id.','.$mapTurn.','.$mapTurn.'); return false;"><img id="End" src="'.l_s('images/historyicons/End_disabled.svg').'" alt="'.l_t('End').'" title="'.l_t('View the map from the most recent turn').'" /></a>'.
				($this->Members->isJoined() ? '<a class="mapnav" href="#" onClick="togglePreview('.$this->id.','.$mapTurn.'); return false;"><img id="Preview" src="images/historyicons/Preview.svg" alt="PreviewMoves" title="Show server side stored orders on the map" /></a>' : '').'
	...
	function showVoteForm($vVote, $vCancel)
	{
		...
		$buf .= '<img id = "modBtnVote" height="16" width="16" src="images/icons/help.svg" alt="Help" title="Help" style="padding: 8px;"/>
