...
// The game comes from its public JSON files and the player's context (utils/api/gameSource.ts), which builds
// what each of these used to fetch from an API route of its own. The state is kept in the same three parts:
// the overview (the game's settings and members), the data (the board as it stands, and the player's orders)
// and the status (the history).
export const fetchGameData = createAsyncThunk(
  "game/fetchData",
  async (queryParams: { countryID?: string; gameID: string }) =>
    loadData(queryParams.gameID),
);

export const fetchGameOverview = createAsyncThunk(
  "game/fetchOverview",
  async (queryParams: { gameID: string }) => loadOverview(queryParams.gameID),
);

export const fetchGameStatus = createAsyncThunk(
  "game/fetchStatus",
  async (queryParams: { countryID?: string; gameID: string }) =>
    loadStatus(queryParams.gameID),
);
...
export const fetchGameMessages = createAsyncThunk(
  ...
  async (queryParams: {
    ...
    countryID?: string;
    sinceTime?: string;
  }) => loadMessages(queryParams.gameID, queryParams.sinceTime),
);

export const fetchPlayerActiveGames = createAsyncThunk(
  "game/fetchActiveGames",
...
export const setVoteStatus = createAsyncThunk(
  ...
  async (queryParams: {
    ...
    invalidateContext(); // Its copy of this player's votes is now out of date
    return data;
  },
);
...
export const markBackFromLeft = createAsyncThunk(
  ...
  async (queryParams: { countryID: string; gameID: string }) => {
    ...
    invalidateContext(); // Its copy of this player's status is now out of date
    return data;
  },
);
...
export const saveOrders = createAsyncThunk(
  ...
  async (data: OrderSubmission, thunkAPI) => {
    ...
    let response;
    invalidateContext(); // Its copy of this player's orders and order status is about to be out of date
    try {
...
// Loading the overview is what starts everything else: WDMainController sees needsGameData set when it
// arrives, and loads the rest of the game
