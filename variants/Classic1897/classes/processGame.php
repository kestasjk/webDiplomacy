...
class Classic1897Variant_processGame extends processGame {
	...
	protected function updateOwners()
	{
		...
		// Empty non-Owned Territories at the end of the first build phase...
		if ( $this->phase == 'Builds' && $this->turn==0 )
		{		
			$DB->sql_put(
				"DELETE FROM wD_TerrStatus WHERE gameID= ".$this->id);
			$DB->sql_put(
				"INSERT INTO wD_TerrStatus (gameID, terrID, countryID, occupyingUnitID )
				SELECT u.gameID, u.terrID as terrID, u.countryID as countryID, u.id as occupyingUnitID 
				FROM wD_Units u
				WHERE u.gameID = ".$this->id);
