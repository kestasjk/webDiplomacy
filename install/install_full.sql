...
-- Anonymous games, press-types
 ...
 UPDATE wD_Misc SET value=91 WHERE name = 'Version';UPDATE wD_Notices SET `text`=REPLACE(REPLACE(`text`,'\\''',''''),'\\"','"');
...
UPDATE wD_Misc SET value=92 WHERE name = 'Version';DELETE FROM wD_Moves;
...
UPDATE wD_Misc SET `value`=95 WHERE `name`='Version';ALTER TABLE wD_Members ADD `orderStatus` set('None','Saved','Completed','Ready') NOT NULL default 'None';
...
UPDATE wD_Misc SET `value`=96 WHERE `name`='Version';ALTER TABLE `wD_Games` ADD `variantID` TINYINT UNSIGNED NOT NULL FIRST;
...
UPDATE wD_Misc SET `value`=97 WHERE `name`='Version';/*ALTER TABLE `wD_Users` ADD `activeHours` CHAR(24) NOT NULL DEFAULT '000000000000000000000000' AFTER `points`;*/
...
UPDATE `wD_Misc` SET `value` = '104' WHERE `name` = 'Version';ALTER TABLE `wD_PointsTransactions` CHANGE `type` `type` ENUM( 'Supplement', 'Bet', 'Won', 'Returned', 'Trigger', 'Correction' ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;
...
UPDATE `wD_Misc` SET `value` = '132' WHERE `name` = 'Version';ALTER TABLE `wD_Games`  ADD `directorUserID` MEDIUMINT UNSIGNED NOT NULL DEFAULT '0';
